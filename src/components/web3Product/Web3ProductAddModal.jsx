...
const Web3ProductAddModal = ({ onClose, handleWeb3ProductAdded }) => {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (response.status !== 200) throw new Error("Rasm yuklashda xatolik");

      setAttachmentId(response.data);
    } catch (err) {
      notifyError("Rasmini yuklashda xatolik yuz berdi.");
      setError("Rasmini yuklashda xatolik yuz berdi.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !attachmentId) {
      setError("Mahsulot nomi va rasmi mavjud emas");
      return;
    }
