...
const ShowAll = ({ users, deleteUser }) => {
    if (!users || users.length === 0)
        return <p className="text-[50px] mt-10 text-center font-semibold text-gray-500">No users found.</p>;
                    ...
                    {users.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50 text-center">
                            ...
                            <td className="p-2 border flex gap-4 items-center justify-evenly">
                                <button onClick={() => { handleDelete(user.id) }} className="w-full bg-red-400 text-re-900 font-semibold py-2 px-4 rounded-lg shadow-md hover:text-white hover:bg-red-500 transition-all duration-300">
                                    Delete
                                </button>
                                <Link
                                    to={`${user.id}`}
