...
import { deleteById, fetchAuctionsPage } from "../../api/request/admin/auction/main.api";
import { dateFormater, dateFormatterWithTime } from './../../helper/dateFormater';
...
const ManageAuction = () => {
    ...
    const handleAuctionAdded = (newAuction) => {
        if (editingAuction) {
            setAuctions(auctions.map(auction => auction.id === editingAuction.id ? newAuction : auction));
        } else {
            fetchPage(page, searchQuery);
        }
    };
                            ...
                            <table className="w-full mt-5 border-collapse border">
                                <thead>
                                    <tr className="text-center border">
                                        ...
                                        <th className="font-semibold border p-2">{t("auction.name")}</th>
                                        <th className="font-semibold border p-2">{t("auction.gift")}</th>
                                        <th className="font-semibold border p-2">{t("auction.count")}</th>
                                        <th className="font-semibold border p-2">{t("auction.startAuction")}</th>
                                        <th className="font-semibold border p-2">{t("auction.endAuction")}</th>
                                        <th className="font-semibold border p-2">{t("auction.edit")}</th>
                                        <th className="font-semibold border p-2">{t("auction.delete")}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {auctions.map((item) => (
                                        <tr key={item.id} className="text-center border">
                                            ...
                                            <td className="p-2">{dateFormatterWithTime(item.startTime)}</td>
                                            <td className="p-2">{dateFormatterWithTime(item.endTime)}</td>
                                            <td className="p-2">
                                                <button
