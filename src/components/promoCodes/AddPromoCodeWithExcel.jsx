...
const AddPromoCodeWithExcel = ({ onClose, onPromoCodeAdded }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:8080/api/v1/promo-codes/import', {
                method: 'POST',
                body: formData,
            });
            if (!response.ok) {
    ...
    return (
        <div className="w-full">
            <h2 className="text-xl font-bold mb-4">{t("promoCodes.addPromoCode.sTitle")}</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="self-end">
                    ...
                    >
                        {loading ? t("promoCodes.addPromoCode.importing") : t("promoCodes.addPromoCode.addWithExcel")}
                    </button>
                </div>
            </form>
        </div>
