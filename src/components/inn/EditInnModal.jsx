...
const EditInnModal = ({ inn, onClose, editInns }) => {
    ...
    const handleSubmit = async e => {
        ...
        try {
            const res = await editInn(inn.id, { inn: newInn.trim(), storeName: newStore.trim() })
            ...
            <div className="rounded-lg p-6 grid grid-cols-1 gap-6">
                <div className="bg-gray-50 p-6 rounded-lg shadow-md flex flex-col h-full">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={onClose} className="w-full bg-slate-500 text-white font-semibold py-2 rounded-md mt-4 hover:bg-slate-600 transition-all duration-300">Close</button>
                        <button onClick={handleSubmit} type="submit" className="w-full bg-green-500 text-white font-semibold py-2 rounded-md mt-4 hover:bg-green-600 transition-all duration-300">
                            Edit
                        </button>
                    </div>
                </div>
