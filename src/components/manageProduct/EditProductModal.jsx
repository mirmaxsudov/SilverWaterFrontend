...
const EditProductModal = ({ product, onClose, onSave }) => {
    ...
    const [description, setDescription] = useState(product.desc || product.description || "");
    const [selectedCategoryId, setSelectedCategoryId] = useState(product.categoryId || null);

    // Image states: use product.imageId and photo (if available)
    const [imageId, setImageId] = useState(product.imageId || null);
    const [imagePreview, setImagePreview] = useState(
        typeof product.photo === "string" ? product.photo : product.photo ? URL.createObjectURL(product.photo) : null
    );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await $api.put(`/api/v1/products/${product.id}`, updatedProduct);
    ...
    return (
        ...
        >
                    ...
                    <div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-1">One Price</label>
                                    <input
                                        type="number"
                                        className="w-full border rounded p-2"
                                        required
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-1">Blog Price</label>
                                    <input
                                        type="number"
                                        className="w-full border rounded p-2"
                                        required
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <label htmlFor="isDisabled" className="text-sm font-medium">Is Disabled</label>
                                </div>
                            </div>
            ...
            <div className="category-container bg-white w-96 rounded-lg h-full overflow-auto p-6 shadow-lg">
                <h1 className="text-2xl font-semibold mb-4 text-gray-800">Categories</h1>
                <input
